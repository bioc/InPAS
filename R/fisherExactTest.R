...
#' @seealso [singleSampleAnalyze()],[singleGroupAnalyze()], [limmaAnalyze()]
#' @keywords internal
#'
#' @examples
#' path <- file.path(find.package("InPAS"), "inst/extdata")
#' load(file.path(path, "eset.MAQC.rda"))
#' sample_tags <- colnames(eset$PDUI.log2)    
#' res <- fisherExactTest(eset, gp1=sample_tags[1:2], 
#'                        gp2=sample_tags[3:4])
#' 
...
fisherExactTest <- function(UTR3eset, gp1, gp2){
    ...
    cbind(short.mean.gp1, long.mean.gp1, 
