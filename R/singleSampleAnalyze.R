...
#' 
#' @description do APA event analysis for a single sample Using Poisson Hidden Markov models
#' @details the test will be performed by comparing a two-state versus
#' an one-state Poisson Hidden Markov models.
#' 
#' @param UTR3eset the output of [getUTR3eSet()]
#' @importFrom depmixS4 depmix fit posterior getpars
#' @importFrom stats BIC fft fisher.test loess.smooth model.matrix p.adjust pnorm poisson t.test
...
singleSampleAnalyze <- function(UTR3eset){
    ...
    phmm <- function(counts){
        ...
        m1 <- depmix(response=counts~1, nstates=1, 
                     data=as.data.frame(counts), 
                     family=poisson(), 
                     respstart=log(mean(counts)))
        m2 <- depmix(response=counts~1, nstates=2, 
                     data=as.data.frame(counts),
                     family=poisson(), 
