...
#' 
#' extract 3' UTR information from a [GenomicFeatures::TxDb-class] 
#' object. The 3'UTR is defined as the last 3'UTR fragment for 
#' each transcript and it will be cut if there is any overlaps 
#' with other exons.
#' 
...
#' @param MAX_EXONS_GAP maximal gap sizes for distal CP site to downstream
#' exons
#' @import plyranges GenomicRanges IRanges
#' @importFrom dplyr as_tibble mutate filter arrange bind_rows group_by left_join summarise n
#' 
#' @author Jianhong Ou, Haibo Liu
#' @return an object of [GenomicRanges::GRanges-class] with 7 metadata columns:
#' feature (utr3, next.exon.gap, CDS), 
#' annotatedProximalCP (unknown, proximalCP_<coordinate>), 
#' exon (<transcript id>_<index>), transcript, 
#' gene (entrez_id), symbol, truncated (logical).
#' @export
#'
#' @examples
#' if (interactive()){
#' library(EnsDb.Hsapiens.v86)
#' TxDb <-loadDb("C:\\Users\\Haibo\\Desktop/003.MAQC.brain.UHR.TxDb.sqlite")
#' edb <- EnsDb.Hsapiens.v86
#' tx <- parseEnsDb(TxDb, edb)
#' utr3Annotation(tx)
#' }
                           ...
                           MAX_EXONS_GAP=10000)
{
    ...
    utr3.last <- assign_feature(utr3.last, feature_alt = "lastutr3") %>%
        plyranges::filter(feature == "lastutr3", width > 1) %>% unique()
    ...
    {
        utr3.last <- gr
        utr3.last <- utr3.last %>% unique() %>%
            plyranges::mutate(start.utr3.last = paste(seqnames, start,
                                                      gene, sep = ":"),
                              end.utr3.last = paste(seqnames, end, 
                                                    gene, sep = ":"))
        
        start.dup <- with(utr3.last, 
                          unique(start.utr3.last[duplicated(start.utr3.last) & 
                                                     as.character(strand(utr3.last))=="+"]))
        end.dup <- with(utr3.last, 
        ...
        if(length(utr3.last.dup) > 0){
            ...
            utr3.last.dup <- utr3.last.dup %>% 
                plyranges::mutate(dup.group = 
                                      ifelse(strand == "+", 
                                      start.utr3.last, end.utr3.last)) %>% 
                plyranges::select(-c(start.utr3.last, end.utr3.last)) %>%
                data.frame() %>% dplyr::as_tibble() %>% 
            ...
            annotation <- lapply(utr3_end, function(.x){
                label <- paste("proximalCP", 
                               paste(.x[-1], collapse="_"), sep="_")
            })
            annotation <- do.call("c", annotation)
            utr3.last.dup <- utr3.last.dup %>% 
                dplyr::filter(!duplicated(dup.group)) %>% 
                dplyr::mutate(annotatedProximalCP = 
                                  annotation[dup.group]) %>%
                dplyr::select(-dup.group) %>% as_granges()
    ...
    utr3.last_overlap_split <- lapply(uniq_pairs, function(.x){
        ...
        if (length(unique(gr$gene)) > 1)
        {
            ...
            if (as.character(strand(gr_reduced)) == "+")
            {
                ...
                truncated <- c(rep(TRUE, length(st) -1), 
                ...
                truncated <- c(start_sort$truncated[1], 
            ...
            gr <- GRanges(data.frame(seqnames = 
                                             seqnames(gr_reduced),
                              start = segment_start,
                              end = segment_end,
                              strand = strand(gr_reduced)),
    ...
    ################################################################### 
    utr3.last.sameStart <- utr3.last[grepl("proximalCP", 
    ...
    gaps <- tx %>% plyranges::reduce_ranges() %>% 
    ...
    utr3.clean.ext1 <- utr3.clean %>% plyranges::filter(!truncated) %>%
    ...
    start(next.exons.gap)[wid & as.character(strand(next.exons.gap))=="-"] <- 
            ...
            wid & as.character(strand(next.exons.gap))=="-"] - MAX_EXONS_GAP + 1
    ...
    CDS.last <- CDS.last[match(utr3.clean$transcript, 
                               CDS.last$transcript)]
