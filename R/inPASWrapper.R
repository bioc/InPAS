...
#' site in one step
#'
#' @param bedgraphs The file names of bedgraphs generated by 
...
#' @param hugeData is s this dataset consume too much memory? 
#' if it is TRUE, the coverage will be saved into tempfiles.
#' @param ... parameters can be passed into tempfile. This is 
...
#' @param window_size window size for novel distal position
...
#' @param cutStart how many nucleotides should be removed from 
#' the start before search, 0.1 means 10 percent.
#' @param cutEnd how many nucleotides should be removed 
#' from the end before search, 0.1 means 10 percent.
#' @param coverage_threshold cutoff threshold for coverage in the 
#' region of short form
#' @param long_coverage_threshold cutoff threshold for coverage 
#' in the region of long form
#' @param background the range for calculating cutoff threshold 
#' of local background
#' @param adjust_distal_polyA_end If true, adjust distal polyA 
...
#' @param classifier_cutoff This is the cutoff used to assign 
...
#' @param method a character(1) vector. The available test method are #' "limma", "fisher.exact", "singleSample", "singleGroup"
...
#' @param dPDUI_cutoff cutoff value for differential 
...
#' @import GenomicRanges IRanges BiocParallel
#'
#' @examples
#' if(interactive()){
...
inPASWrapper <- function(bedgraphs, genome, removeScaffolds = FALSE,
                  utr3, txdb=NA,
                  tags, hugeData=FALSE, ...,
                  gp1, gp2,
                  window_size=100, 
